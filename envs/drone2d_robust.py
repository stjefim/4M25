...
GROUND_DEF = Box2D.b2FixtureDef(shape=Box2D.b2PolygonShape(box=(WORLD_SIZE / 2, 0.1)))
...
class Drone2DRobust(gym.Env):
    ...
    def __init__(self, reward_func, multiple_obj=False, render_mode=None, action_type=ACTION_FORCES, spawn_position=(0, 0), initial_target_pos=None, relative_target=False):
        ...
        dims_min = np.array([-WORLD_SIZE / 2, -0.25, -np.pi, -np.pi, -MAX_SPEED, -MAX_SPEED, -MAX_ANGULAR_SPEED, -MAX_ANGULAR_SPEED, -TARGET_MAX_X, 0])
        dims_max = np.array([WORLD_SIZE / 2, 4.75, np.pi, np.pi, MAX_SPEED, MAX_SPEED, MAX_ANGULAR_SPEED, MAX_ANGULAR_SPEED, TARGET_MAX_X, TARGET_MAX_Y])
    ...
    def step(self, action):
        ...
        # Apply engine forces
        self.drone.ApplyForce(force=self.drone.GetWorldVector([0., float(self.action[0])]), point=self.drone.GetWorldPoint([-0.2, 0.]), wake=True)
    ...
    def _draw_force(self, canvas, local_pos, action_index):
        ...
        if self.action is not None:
            endf = np.round(self._coord_transform(self.drone.GetWorldPoint(local_pos + np.array([0., 0.2]) * self.action[action_index] / MAX_FORCE))).astype(int)
...
def main():
    ...
    if JOYSTICK:
        env = Drone2D(render_mode="human", action_type=Drone2D.ACTION_FORCE_AND_TORQUE, reward_func=InverseDistance(), multiple_obj=True)
        joystick = Joystick()
    else:
        env = Drone2D(render_mode="human", action_type=Drone2D.ACTION_FORCES, reward_func=InverseDistance(), multiple_obj=True)
